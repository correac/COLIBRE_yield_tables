...
def read_AGB_COLIBRE(metallicity_flag, metallicity):
    ...
    with h5py.File('./data/AGB.hdf5', 'r') as data_file:
        Masses = data_file["Masses"][:]
        Yield_Z0p007 = data_file["/Yields/Z_"+metallicity_flag+"/Yield"][:][:]
        Ejected_mass_Z0p007 = data_file["/Yields/Z_"+metallicity_flag+"/Ejected_mass"][:]
    ...
    for i in range(num_mass_bins):
        stellar_yields[:, i] = Yield_Z0p007[indx, i] + (metallicity / total_mass_fraction) * mass_fraction * Ejected_mass_Z0p007[i]
...
def plot_AGB():
    ...
    width = 0.1
    opacity = 0.8
    plt.bar(index, colibre_Z0, bar_width, alpha=1, color='darkblue', label='Z=0.007')
    plt.bar(index + bar_width, colibre_Z1, bar_width, alpha=opacity, color='tab:blue', label='Z=0.014')
    plt.bar(index + 2 * bar_width, colibre_Z2, bar_width, alpha=1, color='tab:orange', label='Z=0.03')
    plt.bar(index + 3 * bar_width, colibre_Z3, bar_width, alpha=opacity, color='lightgreen', label='Z=0.06')
    plt.bar(index + 4 * bar_width, colibre_Z4, bar_width, alpha=opacity, color='darkgreen', label='Z=0.1')
