...
def apply_linear_extrapolation():
    ...
    Z_bins = np.array([0.0500, 0.0200, 0.0080, 0.0040, 0.0010, 0.0000])
    ...
    with h5py.File('./data/SNII_linear_extrapolation.hdf5', 'w') as data_file:
        ...
        MH = data_file.create_dataset('Masses', data=M_extrapolated)
        MH.attrs["Description"]=np.string_("Mass bins in units of Msolar")

        MH = data_file.create_dataset('Metallicities', data=Z_bins)
        MH.attrs["Description"]=np.string_("Metallicity bins")

        MH = data_file.create_dataset('Number_of_metallicities', data=len(Z_bins))

        MH = data_file.create_dataset('Number_of_masses', data=len(M_extrapolated))
        ...
        for Zi in Z_list:
            ...
            MH = Z00.create_dataset('Ejected_mass_in_ccsn', data=mej_ccsn_extrapolated)
            MH = Z00.create_dataset('Ejected_mass_in_winds', data=mej_wind_extrapolated)
            MH = Z00.create_dataset('Total_Mass_ejected', data=mass_loss_ccnn_extrapolated)
