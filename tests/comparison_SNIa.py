...
def read_AGB_EAGLE(metallicity_flag, metallicity):
...
def read_SNIa_EAGLE():
    ...
    with h5py.File('./data/EAGLE_yieldtables/SNIa.hdf5', 'r') as data_file:
        ...
        total_yields = data_file["Total_Metals"]
...
def read_SNIa_COLIBRE():
    ...
    integral_mej = simpson(IMF * NSNIa * masses, x=masses)
    # integral_all = simpson(IMF * masses, x=masses)
...
def read_SNII_EAGLE(metallicity_flag, metallicity):
...
def read_SNIa_COLIBRE(flag):

    if flag == 1:
        with h5py.File('../data/SNIa.hdf5', 'r') as data_file:
            stellar_yields = data_file["Yield"][:]
    if flag == 2:
        with h5py.File('../data/LeungNomoto2018/SNIa_benchmarkLeungNomoto2018.hdf5', 'r') as data_file:
            stellar_yields = data_file["Yield"][:]
    if flag == 3:
...
def plot_SNIa(flag):
    ...
    colibre = read_SNIa_COLIBRE(flag)
...
def make_comparison():
    ...
    ax.text(0.1, 0.96, 'SNIa yields/Kobayashi et al. (2020)', transform=ax.transAxes,
    ...
    props = dict(boxstyle='round', fc='white', ec='black', alpha=1)
    ax.text(0.1, 0.96, 'SNIa yields/Leung \& Nomoto (2018)', transform=ax.transAxes,
            fontsize=11, verticalalignment='top', bbox=props)
    props = dict(boxstyle='round', fc='white', ec='white', alpha=0)
    ax.text(0.1, 0.88, 'Benchmark model', transform=ax.transAxes,
            fontsize=11, verticalalignment='top', bbox=props)
    ...
    ax.text(0.1, 0.96, 'SNIa yields/Leung \& Nomoto (2018)', transform=ax.transAxes,
            fontsize=11, verticalalignment='top', bbox=props)
    props = dict(boxstyle='round', fc='white', ec='white', alpha=0)
    ax.text(0.1, 0.88, 'W7 model', transform=ax.transAxes,
            fontsize=11, verticalalignment='top', bbox=props)
    ...
    plt.savefig('./Comparison_SNIa_yields.png', dpi=300)
