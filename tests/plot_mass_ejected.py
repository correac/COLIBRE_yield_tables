...
def read_SNII_EAGLE(Z,max_range):
    ...
    new_mass_range = np.arange(8, max_range, 0.1)
    new_imf_range = np.zeros(len(new_mass_range))
    new_stellar_yields_range = np.zeros(len(new_mass_range))
    eagle = np.zeros(9)
    for i in range(0,9):
...
if __name__ == "__main__":
    ...
    AGB_mej = read_AGB_COLIBRE(metallicity)
    CCSN_mej, winds_mej, CCSN_mej_total = read_SNII_COLIBRE(metallicity)
    AGB_mej /= IMF_int
    CCSN_mej /= IMF_int
    winds_mej /= IMF_int
    CCSN_mej_total /= IMF_int
    CCSN_Eagle = read_SNII_EAGLE(metallicity,100)
    CCSN_Eagle /= IMF_int
    CCSN_Eagle_2 = read_SNII_EAGLE(metallicity,40)
    CCSN_Eagle_2 /= IMF_int
    ...
    width = 0.1
    opacity = 0.8
    plt.bar(index, AGB_mej, bar_width, alpha=1, color='darkblue', label='AGB')
    plt.bar(index + bar_width, CCSN_mej, bar_width, alpha=opacity, color='tab:blue', label='CCSN/Explosion')
    plt.bar(index + 2 * bar_width, winds_mej, bar_width, alpha=opacity, color='tab:orange', label='CCSN/Pre-phase')
    plt.bar(index + 3 * bar_width, CCSN_mej_total, bar_width, alpha=opacity, color='crimson', label='CCSN/Total')
    ...
    # plt.tight_layout()
    plt.savefig('Comparison_mass_ejected_Z_0p001.png', dpi=300)
    ...
    width = 0.1
    opacity = 0.8
    plt.bar(index, CCSN_Eagle, bar_width, alpha=1, color='darkblue', label='CCSN/Eagle Yields (8-100 M$_{\odot}$)')
    plt.bar(index + bar_width, CCSN_Eagle_2, bar_width, alpha=0.5, color='darkblue', label='CCSN/Eagle Yields  (8-40 M$_{\odot}$)')
    plt.bar(index + 2 * bar_width, CCSN_mej, bar_width, alpha=opacity, color='tab:blue', label='CCSN/Explosion/Colibre Yields')
    plt.bar(index + 3 * bar_width, winds_mej, bar_width, alpha=opacity, color='tab:orange', label='CCSN/Pre-phase/Colibre Yields')
    plt.bar(index + 4 * bar_width, CCSN_mej_total, bar_width, alpha=opacity, color='crimson', label='CCSN/Total/Colibre Yields')
    ...
    # plt.tight_layout()
    plt.axis([-0.5, 13.5, 1e-5, 10])
    plt.yscale('log')
    plt.legend(loc=[0.3, 0.6], labelspacing=0.2, handlelength=0.8,
               handletextpad=0.3, frameon=False, columnspacing=0.4,
               ncol=1, fontsize=10)
    ...
    ax.tick_params(direction='in', axis='both', which='both', pad=4.5)
    # plt.tight_layout()
    plt.savefig('Comparison_CCSN_mass_ejected_Z_0p001.png', dpi=300)